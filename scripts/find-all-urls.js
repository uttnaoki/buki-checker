...
for (const [weaponId, possibleNames] of Object.entries(weaponNames)) {
  let matchingPath = null;

  // 各候補名で検索
  for (const name of possibleNames) {
    // まず正確なマッチを試す（_2D_Currentなし）
    let pattern = `S3_Weapon_Main_${name}.png`;
    matchingPath = allPaths.find(path => path.endsWith(pattern));

    if (!matchingPath) {
      // _2D_Current付きで試す
      pattern = `S3_Weapon_Main_${name}_2D_Current.png`;
      matchingPath = allPaths.find(path => path.endsWith(pattern));
    }
  ...
  if (matchingPath) {
    ...
    result[weaponId] = url;
    found++;
    console.log(`✓ ${weaponId}: ${matchingPath.split('/').pop()}`);
  } else {
    notFound.push({ id: weaponId, names: possibleNames });
    console.log(`✗ ${weaponId}: ${possibleNames.join(', ')} (not found)`);
  }
}

console.log(`\n========================================`);
console.log(`見つかった武器: ${found}/${Object.keys(weaponNames).length}`);
...
if (notFound.length > 0) {
  console.log('\n【見つからなかった武器】');
  notFound.forEach(w => {
    console.log(`  ${w.id}: ${w.names.join(', ')}`);
    // 類似のファイル名を検索
    const searchTerm = w.names[0].replace(/_/g, '').toLowerCase();
    const similar = allPaths.filter(p =>
      p.toLowerCase().includes(searchTerm.substring(0, 10))
    ).slice(0, 3);
    if (similar.length > 0) {
      console.log(`    類似: ${similar.map(s => s.split('/').pop()).join(', ')}`);
    }
  });
}

// JSONとして保存
const outputPath = __dirname + '/weapon-urls-final.json';
